...
export class EmailTemplatesController {
  list = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const items = await db('email_templates').where({ user_id: userId }).orderBy('nome', 'asc');
  ...
  create = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (data.padrao) {
        await db('email_templates').where({ user_id: userId }).update({ padrao: false });
      }

      const [item] = await db('email_templates')
  ...
  delete = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const deleted = await db('email_templates').where({ id, user_id: userId }).delete();
